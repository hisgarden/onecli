...
export const app = new Elysia()
  ...
  .onError(({ error, set }) => {
    ...
    // ZodError from `schema.parse(body)` in route handlers. We use duck
    // typing (name + issues array) instead of `instanceof ZodError` so we
    // don't need zod as an apps/api dependency, and so cross-module-boundary
    // instanceof issues never bite us if zod versions ever drift.
    if (
      ...
      const issues = (
        error as unknown as { issues: Array<{ message?: string }> }
      ).issues;
      set.status = 400;
      const message =
        issues
