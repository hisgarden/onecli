...
export function createMockDb() {
  ...
  function createBuilder(method: string, table?: string): any {
    ...
    // execute() is used for fire-and-forget inserts/updates/deletes — it does
    // NOT consume from the queue, so route tests can queue results for the
    // executeTakeFirst*() reads they care about without padding placeholders
    // for every intervening insert. (This diverges from the services package's
    // copy of mock-db.ts on purpose; the api routes have many more inserts
    // per request than services do.)
    chain.execute = mock(() => Promise.resolve([]));
  ...
  const db = {
    ...
    /**
     * Reset queued results and call history. Used by route tests to keep a
     * stable mockDb identity (so import bindings don't break) while still
